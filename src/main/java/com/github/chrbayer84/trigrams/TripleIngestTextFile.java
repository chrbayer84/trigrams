...
public class TripleIngestTextFile
{
    private static final int N = 3;

    private static final Pattern spacePattern = Pattern.compile("\\s");

    static class StringLineProcessor implements LineProcessor<String>
    {
        private final BatchWriter writer;

        private final LinkedList<String> before = Lists.newLinkedList();

        StringLineProcessor(BatchWriter writer)
        {
            this.writer = writer;
        }
        ...
        public boolean processLine(String line) throws IOException
        {
            ...
            int start = (currentLine.length - N > 0) ? currentLine.length - N
                    : 0;
    ...
    public void readFromTextFile(File file, final BatchWriter writer)
            throws TableNotFoundException, MutationsRejectedException,
            IOException
    {
        Files.readLines(file, Charsets.UTF_8, new StringLineProcessor(writer));
    }
}
